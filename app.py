...
# ── SOP PORTFOLIO ──
    ...
    st.markdown(
        f"<div style='background:transparent;border-left:3px solid {TIDE_POOL};"
        f"padding:8px 14px;margin-bottom:20px;font-size:0.8rem;color:{TEXT_MUTED};line-height:1.5'>"
        f"<b style='color:{TIDE_POOL}'>Grade (/10)</b> = how ready the SOP is to adopt (clarity, completeness, automation). "
        f"<b style='color:{TIDE_POOL}'>Hours/year</b> = value at scale (time saved x frequency x team size). "
        f"Sweet spot is both high. Low grade + high hours = improve the docs."
        f"</div>",
        unsafe_allow_html=True,
    )
